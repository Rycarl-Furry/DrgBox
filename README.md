...
Windows 版功能最完整；Linux 版支持分类管理、托盘和常规工具启动，macOS 版支持分类管理和常规工具启动。Windows 原生全局快捷键、EXE 图标提取及管理员启动在其他系统中不可用。由于 Wails v2 与当前托盘库的 macOS `AppDelegate` 冲突，macOS 版关闭窗口时直接退出，暂不提供托盘常驻。
![首页](./img/2.png)
## 项目结构
